...
const boardMembers: TeamMember[] = [
  ...
  },
  {
    ...
    bio: 'Co-Founder of VOYlegal, a legal search firm that works with the top law firms in the world. As an entrepreneur of various ventures, Miten brings business expertise and his expansive network to nonprofit governance. He is committed to driving sustainable growth and impact for youth development programs.',
...
function TeamMemberCard({ member, index, onClick }: { member: TeamMember; index: number; onClick: () => void }) {
  return (
    <motion.button
      ...
      onClick={onClick}
      className="group text-center cursor-pointer bg-transparent border-none"
...
function FeaturedMemberCard({ member, index, onClick }: { member: TeamMember; index: number; onClick: () => void }) {
  return (
    <motion.button
      ...
      onClick={onClick}
      className="group glass border border-orange-primary/20 rounded-3xl p-8 md:p-12 cursor-pointer hover:border-orange-primary transition-all duration-400 hover:-translate-y-2 hover:shadow-glow text-left"
      aria-label={`View details for ${member.name}`}
    >
